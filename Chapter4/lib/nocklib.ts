...
module.exports = {
    ...
    createUser: function(inUsername, inEmail, inPassword, callback) {
        var user = {
            username: inUsername, email: inEmail,
...
function encryptPassword(inPlainText) {
    return crypto.createHash('md5').update(inPlainText).digest('hex');
}
